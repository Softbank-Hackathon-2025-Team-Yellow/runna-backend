...
def get_workspaces(
    ...
    return create_success_response({"workspaces": [w.model_dump() for w in workspace_responses]})
...
def get_workspace_functions(
    ...
    try:
        ...
        functions = db.query(Function).filter(Function.workspace_id == workspace_id).all()
